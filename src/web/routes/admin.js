...
let sendJobStatus = { running: false, sent: 0, failed: 0, total: 0, startedAt: null };
...
// ─── EXPIRING: Envio com controle de enviados hoje ────────────────────────────
...
router.get('/expiring', requireAuth, async (req, res) => {
  ...
  const notSentToday = subs.rows.filter(s => !s.renewal_sent_at || new Date(s.renewal_sent_at) < today);
  const sentToday    = subs.rows.filter(s => s.renewal_sent_at && new Date(s.renewal_sent_at) >= today);
...
router.post('/expiring/send', requireAuth, async (req, res) => {
  if (sendJobStatus.running) {
    return res.redirect('/admin/expiring?error=job_already_running');
  }
  ...
  const subs = await db.query(\`
    ...
    WHERE s.status = 'active'
      AND s.expires_at > NOW()
      AND s.expires_at <= NOW() + INTERVAL '3 days'
      AND (u.renewal_sent_at IS NULL OR u.renewal_sent_at < $1)
  \`, [today.toISOString()]);

  sendJobStatus = { running: true, sent: 0, failed: 0, total: subs.rows.length, startedAt: new Date() };

  res.redirect(\`/admin/expiring?success=job_started_\${subs.rows.length}\`);

  setImmediate(async () => {
    const BATCH_SIZE = 20;
    const DELAY_BETWEEN_MSGS = 300;
    ...
    for (let i = 0; i < subs.rows.length; i++) {
      ...
      try {
        ...
        await db.query(
          \`UPDATE users SET renewal_sent_at = NOW() WHERE id = $1\`,
          [sub.user_id]
        );

        sendJobStatus.sent++;
      } catch (e) {
        console.error(\`[expiring/send] Falhou para \${sub.telegram_id}:\`, e.message);
        sendJobStatus.failed++;
      }

      await new Promise(r => setTimeout(r, DELAY_BETWEEN_MSGS));
    ...
    sendJobStatus.running = false;
  });
});

router.get('/expiring/status', requireAuth, (req, res) => {
  res.json(sendJobStatus);
});
