...
router.post('/subscribers/:id/cancel', requireAuth, async (req, res) => {
  ...
  try {
    await bot.telegram.sendMessage(
      sub.telegram_id,
      `❌ *Sua assinatura foi cancelada.* Seu acesso ao grupo foi removido. Se tiver dúvidas, entre em contato com o suporte.`,
      { parse_mode: 'Markdown' }
    );
  } catch (e) {
...
router.post('/subscribers/:id/expiry', requireAuth, async (req, res) => {
  ...
  const subRes = await db.query(
    `SELECT s.*, u.telegram_id, u.first_name FROM subscriptions s JOIN users u ON u.id = s.user_id WHERE s.id = $1 AND s.status = 'active'`,
    [id]
  );
  ...
  try {
    ...
    await bot.telegram.sendMessage(
      sub.telegram_id,
      `📅 *Seu vencimento foi atualizado!* Olá ${sub.first_name}, sua assinatura agora expira em *${dataFormatada}*. ✅`,
      { parse_mode: 'Markdown' }
    );
  } catch (e) {
...
router.post('/broadcast', requireAuth, upload.single('photo'), async (req, res) => {
  ...
  let sent = 0;
  let failed = 0;Revert admin.js to last stable version (82704d4)
